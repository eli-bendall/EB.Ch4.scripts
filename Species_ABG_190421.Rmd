...
DAT <- read_excel("BIOMASS_DATA_2AUG20.xlsx") %>%
  ...
  dplyr::filter(!is.na(total_ag_biomass)) %>%
  dplyr::filter(bg_total > 0) %>%
  
  dplyr::select(veg, co2, watering, species, total_ag_biomass) %>%
...
ggplot(data = DAT) +
  geom_point(aes(x = total_ag_biomass, y = cell_label, colour = species_veg),
...
dat.model <- DAT %>%
  ...
  dplyr::select(y = total_ag_biomass,
                ycopy = total_ag_biomass,
                cell, sp_in_cell) %>%
...
ggABG <- ggplot(data = DAT1, aes(y = cell_label)) +
  geom_point(aes(x = total_ag_biomass, colour = species_veg),
  ...
  labs(x = "Above-ground biomass (g)", y = "Treatment combination", colour = "Species") + 
  
  coord_flip()


ggsave(filename = "total_ag_biomass_cell_species.png")

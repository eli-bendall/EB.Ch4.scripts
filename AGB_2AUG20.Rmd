---
title: "Cell means models_above-ground biomass"
output:
...
cell.stats <- DAT %>% 
  ...
  summarize(nobs = n(),
            mean = mean(total_ag_biomass),
            median = median(total_ag_biomass),
            stdev = sd(total_ag_biomass),
            q25 = quantile(total_ag_biomass, 0.25),
            q75 = quantile(total_ag_biomass, 0.75)) %>%
  ungroup()
...
ggplot(data = DAT, aes(x = veg)) +
  ...
  geom_point(aes(y = total_ag_biomass, colour = co2),
  ...
  labs(x = "Forest type", y = "Biomass (g)",
       title = "Total above_ground biomass",
...
dat <- DAT %>%
  mutate(resid = total_ag_biomass - model2.postmeans[ cell ])
...
AGB.species <- ggplot(data = dat.gg) +
...
DSF.1.sh <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
 
   My_Theme +

    geom_jitter(data = DAT.plot.DSF, aes(y = total_ag_biomass, color = var), 
            height = 0.2,
  ...
  annotate("text", x = 0.8, y = 68, label = "(e)", size = 5, fontface = 2) +
  
  
    scale_y_sqrt(breaks = c(0, 10, 20, 30, 40, 50, 60, 70))+
...
GWD.1.sh <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.GWD, aes(y = total_ag_biomass, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 68, label = "(f)", size = 5, fontface = 2) +
...
WSF.1.sh <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.WSF, aes(y = total_ag_biomass, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  theme(axis.text.y = element_text(size = 12), axis.title.y = element_text(size = 14), axis.title.x = element_text(size = 14)) +
  
  labs(x = NULL, y = "AGB (g)\n") +
  
  annotate("text", x = 0.8, y = 68, label = "(d)", size = 5, fontface = 2) +
...
My_Theme = theme(
  plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
  axis.title.y = element_text(size = 14, face = "bold", hjust = 0.5, vjust = 2.2),
  axis.text.y = element_text(size = 14, margin = unit(c(0.1, 0.1, 0.1, -0.2), "cm")),
  axis.text.x = element_blank(),
  axis.title.x = element_text(size = 14, face = "bold"),
  strip.text.x = element_text(size = 14),
...
DSF.2.sh <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
    ...
    annotate("text", x = 0.6, y = 2.5, label = "(e)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -12.5, label = "-4.96 %", size = 4, fontface = 2) +
...
GWD.2.sh <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
    ...
    annotate("text", x = 0.6, y = 2.5, label = "(f)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -12.5, label = "-2.14 %", size = 4, fontface = 2) +
...
WSF.2.sh <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  labs(x = NULL, y = "\u0394 AGB (g)\n") +

 theme(axis.text.y = element_text(size = 14), axis.title.y = element_text(size = 14)) +
  
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  
    annotate("text", x = 0.6, y = 2.5, label = "(d)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -12.5, label = "-13.92 %", size = 4, fontface = 2) +
...
library(ggpubr)
library(egg)
fig <- egg::ggarrange(WSF.1.sh, DSF.1.sh, GWD.1.sh, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 
...
library(ggpubr)
library(egg)
fig2 <- egg::ggarrange(WSF.2.sh, DSF.2.sh, GWD.2.sh, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 

---
title: "Cell means models_coarse root biomass_14 SEP 2019"
output:
...
cell.stats <- DAT %>% 
  ...
  summarize(nobs = n(),
            ...
            median = median(root_biomass),
            stdev = sd(root_biomass),
            q25 = quantile(root_biomass, 0.25),
            q75 = quantile(root_biomass, 0.75)) %>%
  ungroup()
...
ggplot(data = DAT, aes(x = veg)) +
  ...
  geom_point(aes(y = root_biomass, colour = co2),
...
dat <- DAT %>%
  mutate(resid = root_biomass - model2.postmeans[ cell ])
...
My_Theme = theme(
  ...
  axis.title.y = element_text(size = 12, face = "bold", hjust = 0.5, vjust = 3.7),
...
DSF.1.ro <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
 
   My_Theme +

    geom_jitter(data = DAT.plot.DSF, aes(y = root_biomass, color = var), 
            height = 0.2,
  ...
  geom_segment(aes(xend = var, y = lwr50+0.03, yend = upr50-0.03, color = var),
  ...
  annotate("text", x = 0.8, y = 2.75, label = "e", size = 4, fontface = 2) +
...
GWD.1.ro <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.GWD, aes(y = root_biomass, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 2.75, label = "f", size = 4, fontface = 2) +
...
WSF.1.ro <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.WSF, aes(y = root_biomass, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 2.75, label = "d", size = 4, fontface = 2) +
...
DSF.2.ro <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  #scale_x_discrete(labels = xtext) +
      ...
      annotate("text", x = 0.6, y = 0.5, label = "e", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.6, label = "-4.40 %", size = 3, fontface = 2) +
  
  scale_y_continuous(breaks = c(-0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6), position = "left") +
  coord_cartesian(ylim=c(-0.8, 0.6)) 
### GWD plot 2

GWD.2.ro <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.5, label = "f", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.6, label = "-7.09 %", size = 3, fontface = 2) +
...
WSF.2.ro <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.5, label = "d", size = 4, fontface = 2) +
  
   annotate("text", x = 1.3, y = -0.6, label = "-7.82 %", size = 3, fontface = 2) +
...
fig <- egg::ggarrange(WSF.1.ro, DSF.1.ro, GWD.1.ro, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 
...
fig <- egg::ggarrange(WSF.2.ro, DSF.2.ro, GWD.2.ro, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 

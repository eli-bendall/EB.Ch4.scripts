...
DAT <- read_excel("BIOMASS_DATA_2AUG20.xlsx") %>%
  ...
  dplyr::filter(!is.na(total_biomass)) %>%
  dplyr::filter(bg_total > 0) %>%
  
  dplyr::select(veg, co2, watering, species, total_biomass) %>%
...
ggplot(data = DAT) +
  geom_point(aes(x = total_biomass, y = cell_label, colour = species_veg),
...
ggTB <- ggplot(data = DAT1, aes(y = cell_label)) +
  geom_point(aes(x = total_biomass, colour = species_veg),
  ...
  labs(x = "Total biomass (g)", y = "Treatment combination", colour = "Species") + 
  
  coord_flip()


ggsave(filename = "total_biomass_cell_species.png")

---
title: "Cell means models_below ground biomass"
output:
...
#### IMPORTANT NOTE: If you wish to have the combined figure at the end of this doc (total biomass, above ground biomass and below ground biomass) then you have to run each sequentially. 
...
cell.stats <- DAT %>% 
  ...
  summarize(nobs = n(),
            mean = mean(bg_total),
            median = median(bg_total),
            stdev = sd(bg_total),
            q25 = quantile(bg_total, 0.25),
            q75 = quantile(bg_total, 0.75)) %>%
  ungroup()
...
ggplot(data = DAT, aes(x = veg)) +
  ...
  geom_point(aes(y = bg_total, colour = co2),
  ...
  labs(x = "Forest type", y = "Biomass (g)",
       title = "Total below_ground biomass",
...
dat <- DAT %>%
  mutate(resid = bg_total - model2.postmeans[ cell ])
...
My_Theme = theme(
  ...
  axis.title.y = element_text(size = 12, face = "bold", hjust = 0.5, vjust = 3.2),
...
DSF.1.bg <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
 
   My_Theme +

    geom_jitter(data = DAT.plot.DSF, aes(y = bg_total, color = var), 
            height = 0.2,
  ...
  geom_segment(aes(xend = var, y = lwr50+0.1, yend = upr50-0.1, color = var),
  ...
  annotate("text", x = 0.8, y = 7.5, label = "h", size = 4, fontface = 2) +
...
GWD.1.bg <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.GWD, aes(y = bg_total, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 7.5, label = "i", size = 4, fontface = 2) +
...
WSF.1.bg <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.WSF, aes(y = bg_total, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 7.5, label = "g", size = 4, fontface = 2) +
...
DSF.2.bg <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  annotate("text", x = 0.6, y = 0.75, label = "h", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-1.58 %", size = 3, fontface = 2) +
  
  scale_y_continuous(breaks = c(-2.5, -2, -1.5, -1, -0.5, 0, 0.5, 1), position = "left") +
  coord_cartesian(ylim=c(-2.5, 1)) 

### GWD plot 2

GWD.2.bg <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.75, label = "i", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-32.52 %", size = 3, fontface = 2) +
...
WSF.2.bg <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.75, label = "g", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-35.62 %", size = 3, fontface = 2) +
...
fig <- egg::ggarrange(WSF.1.bg, DSF.1.bg, GWD.1.bg, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 
...
fig <- egg::ggarrange(WSF.2.bg, DSF.2.bg, GWD.2.bg, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 

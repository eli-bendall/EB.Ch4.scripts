...
BGB.species <- ggplot(data = dat.gg) +
...
My_Theme = theme(
  ...
  axis.title.y = element_text(size = 14, face = "bold", hjust = 0.5, vjust = 0.8),
...
DSF.1.bg <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 9.5, label = "(h)", size = 5, fontface = 2) +
...
GWD.1.bg <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 9.5, label = "(i)", size = 5, fontface = 2) +
...
WSF.1.bg <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 9.5, label = "(g)", size = 5, fontface = 2) +
...
DSF.2.bg <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  annotate("text", x = 0.6, y = 0.75, label = "(h)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-1.58 %", size = 4, fontface = 2) +
...
GWD.2.bg <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.75, label = "(i)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-32.52 %", size = 4, fontface = 2) +
...
WSF.2.bg <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.75, label = "(g)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-35.62 %", size = 4, fontface = 2) +

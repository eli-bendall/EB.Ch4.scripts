...
My_Theme = theme(
  ...
  axis.title.y = element_text(size = 12, face = "bold", hjust = 0.5, vjust = 1),
...
DSF.1.rs <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 0.2375, label = "e", size = 4, fontface = 2) +
  
  scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.2, 0.25), labels = dropLeadingZero) +
...
GWD.1.rs <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 0.2375, label = "f", size = 4, fontface = 2) +
...
WSF.1.rs <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 0.2375, label = "d", size = 4, fontface = 2) +
...
DSF.2.rs <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  scale_x_discrete(labels = xtext) +
      ...
      annotate("text", x = 0.6, y = 0.03, label = "e", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.07, label = "-0.01 %", size = 3, fontface = 2) +
  
    scale_y_continuous(labels = scales::percent_format(accuracy = 2, suffix = ""), breaks = c(-0.08, -0.06, -0.04, -0.02, 0, 0.02, 0.04), position = "left") +
...
GWD.2.rs <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.03, label = "f", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.07, label = "-3.58 %", size = 3, fontface = 2) +
...
WSF.2.rs <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  theme(axis.text.y = element_text(size = 10), axis.title.y = element_text(size = 12, vjust = 3)) +
  
  scale_x_discrete(labels = xtext) +
  
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  
      annotate("text", x = 0.6, y = 0.03, label = "d", size = 4, fontface = 2) +
  
     annotate("text", x = 1.3, y = -0.07, label = "-1.77 %", size = 3, fontface = 2) +

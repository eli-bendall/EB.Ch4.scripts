...
crown.species <- ggplot(data = dat.gg) +
...
My_Theme = theme(
  ...
  axis.title.y = element_text(size = 14, face = "bold", hjust = 0.5, vjust = 4),
  axis.text.y = element_text(size = 12, margin = unit(c(0.1, 0.1, 0.1, -0.2), "cm")),
...
DSF.1.c <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 5.2, label = "(e)", size = 5, fontface = 2) +
  
  
  scale_y_sqrt(breaks = c(0, 0.5, 1.5, 2.5, 3.5, 4.5, 5.5)) +
...
GWD.1.c <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 5.2, label = "(f)", size = 5, fontface = 2) +
...
WSF.1.c <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 5.2, label = "(d)", size = 5, fontface = 2) +
...
DSF.2.c <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  scale_x_discrete(labels = xtext) +
      ...
      annotate("text", x = 0.55, y = 0.7, label = "(e)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.7, label = "12.70 %", size = 4, fontface = 2) +
...
GWD.2.c <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.55, y = 0.7, label = "(f)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.7, label = "-2.69 %", size = 4, fontface = 2) +
...
WSF.2.c <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  theme(axis.text.y = element_text(size = 14), axis.title.y = element_text(size = 14, vjust = 3, hjust = 0.7)) +
  
  scale_x_discrete(labels = xtext) +
  
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  
      annotate("text", x = 0.55, y = 0.7, label = "(d)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.7, label = "9.16 %", size = 4, fontface = 2) +

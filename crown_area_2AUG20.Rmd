---
title: "Cell means models_crown area_14 SEP 2019"
output:
...
cell.stats <- DAT %>% 
  ...
  summarize(nobs = n(),
            mean = mean(crown_ell2),
            median = median(crown_ell2),
            stdev = sd(crown_ell2),
            q25 = quantile(crown_ell2, 0.25),
            q75 = quantile(crown_ell2, 0.75)) %>%
  ungroup()
...
ggplot(data = DAT, aes(x = veg)) +
  ...
  geom_point(aes(y = crown_ell2, colour = co2),
  ...
  labs(x = "Forest type", y = "Height (cm)",
       title = "Crown",
...
dat <- DAT %>%
  mutate(resid = crown_ell2 - model2.postmeans[ cell ])
...
My_Theme = theme(
  ...
  axis.title.y = element_text(size = 12, face = "bold", hjust = 0.5, vjust = 5.75),
...
DSF.1.c <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
 
   My_Theme +

    geom_jitter(data = DAT.plot.DSF, aes(y = crown_ell2, color = var), 
            height = 0.2,
  ...
  geom_segment(aes(xend = var, y = lwr50+0.05, yend = upr50-0.05, color = var),
               size = 4.75, show.legend = FALSE) +
  
  scale_color_manual(values = cols) +
 
  theme(axis.text.y = element_blank(), axis.text.x = element_text(size = 9, hjust=0), axis.title.y = element_text(size = 12), axis.title.x = element_text(size = 12)) +
  
  labs(x = NULL, y = NULL) +
  
  scale_x_discrete(labels=xtext) +
  
  annotate("text", x = 0.8, y = 3.75, label = "e", size = 4, fontface = 2) +
...
GWD.1.c <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.GWD, aes(y = crown_ell2, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 3.75, label = "f", size = 4, fontface = 2) +
...
WSF.1.c <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.WSF, aes(y = crown_ell2, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  theme(axis.text.x = element_text(size = 9, hjust=0), axis.title.y = element_text(size = 12), axis.title.x = element_text(size = 12)) +
  
  labs(x = "", y = expression(bold("Crown area"~(m^{2}))))  +
  
  scale_x_discrete(labels=xtext) +
  
  annotate("text", x = 0.8, y = 3.75, label = "d", size = 4, fontface = 2) +
...
DSF.2.c <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  scale_x_discrete(labels = xtext) +
      ...
      annotate("text", x = 0.6, y = 0.7, label = "e", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.7, label = "12.70 %", size = 3, fontface = 2) +
  
  scale_y_continuous(breaks = c(-0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6, 0.8), position = "left") +
  coord_cartesian(ylim=c(-0.8, 0.8)) 
### GWD plot 2

GWD.2.c <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      ...
      annotate("text", x = 0.6, y = 0.7, label = "f", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.7, label = "-2.69 %", size = 3, fontface = 2) +
...
WSF.2.c <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  labs(x = "", y = expression(bold("\u0394 area"~(m^{2})~"\n"))) +

  theme(axis.text.y = element_text(size = 10), axis.title.y = element_text(size = 12, vjust = 4, hjust = 0.7)) +
  
  scale_x_discrete(labels = xtext) +
  
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  
      annotate("text", x = 0.6, y = 0.7, label = "d", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.7, label = "9.16 %", size = 3, fontface = 2) +
...
fig <- egg::ggarrange(WSF.1.c, DSF.1.c, GWD.1.c, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 
...
fig <- egg::ggarrange(WSF.2.c, DSF.2.c, GWD.2.c, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 

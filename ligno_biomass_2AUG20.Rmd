---
title: "Cell means models_ligno biomass_14 SEP 2019"
output:
...
DAT <- read_excel("AG_BIOMASS_FINAL_OCT.xlsx") %>%
  ...
  filter(!is.na(ligno_total)) %>%
  filter(ligno_total > 0)
...
cell.stats <- DAT %>% 
  ...
  summarize(nobs = n(),
            mean = mean(ligno_total),
            median = median(ligno_total),
            stdev = sd(ligno_total),
            q25 = quantile(ligno_total, 0.25),
            q75 = quantile(ligno_total, 0.75)) %>%
  ungroup()
...
ggplot(data = DAT, aes(x = veg)) +
  ...
  geom_point(aes(y = ligno_total, colour = co2),
...
dat <- DAT %>%
  mutate(resid = ligno_total - model2.postmeans[ cell ])
...
```{r}
## Calculate predicted difference due to aCO2
...
DSF.1.lg <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
 
   My_Theme +

    geom_jitter(data = DAT.plot.DSF, aes(y = ligno_total, color = var), 
            height = 0.2,
  ...
  geom_segment(aes(xend = var, y = lwr50+0.006, yend = upr50-0.006, color = var),
  ...
  annotate("text", x = 0.8, y = .9, label = "b", size = 4, fontface = 2) +
  
  scale_y_continuous(breaks = c(0, .25, .5, .75, .99), labels = ff_num(zero = 0)) +
...
GWD.1.lg <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.GWD, aes(y = ligno_total, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = .9, label = "c", size = 4, fontface = 2) +
...
WSF.1.lg <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.WSF, aes(y = ligno_total, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = .9, label = "a", size = 4, fontface = 2) +
...
DSF.2.lg <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  annotate("text", x = 0.6, y = 0.35, label = "b", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.45, label = "22.20 %", size = 3, fontface = 2) +
  
    scale_y_continuous(breaks = c(-0.5, -0.4, -0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4), position = "left") +
...
GWD.2.lg<- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  annotate("text", x = 0.6, y = 0.35, label = "c", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.45, label = "-6.35 %", size = 3, fontface = 2) +
...
WSF.2.lg <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  annotate("text", x = 0.6, y = 0.35, label = "a", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -0.45, label = "-43.29 %", size = 3, fontface = 2) +
...
library(ggpubr)
library(egg)
fig <- egg::ggarrange(WSF.1.lg, DSF.1.lg, GWD.1.lg, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 
...
library(ggpubr)
library(egg)
fig2 <- egg::ggarrange(WSF.2.lg, DSF.2.lg, GWD.2.lg, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 

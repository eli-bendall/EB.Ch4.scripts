---
title: "Cell means models_plant height"
output:
...
cell.stats <- DAT %>% 
  ...
  summarize(nobs = n(),
            mean = mean(f_height),
            median = median(f_height),
            stdev = sd(f_height),
            q25 = quantile(f_height, 0.25),
            q75 = quantile(f_height, 0.75)) %>%
  ungroup()
...
ggplot(data = DAT, aes(x = veg)) +
  ...
  geom_point(aes(y = f_height, colour = co2),
...
dat <- DAT %>%
  mutate(resid = f_height - model2.postmeans[ cell ])
...
height.species <- ggplot(data = dat.gg) +
...
DSF.1.ph <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
 
   My_Theme +

    geom_jitter(data = DAT.plot.DSF, aes(y = f_height, color = var), 
            height = 0.2,
  ...
  geom_segment(aes(xend = var, y = lwr50+0.8, yend = upr50-0.8, color = var),
  ...
  annotate("text", x = 0.8, y = 130, label = "(b)", size = 5, fontface = 2) +
  
  scale_y_sqrt(breaks = c(20, 40, 60, 80, 100, 120, 140)) +
  coord_cartesian(ylim=c(20, 135)) 
...
GWD.1.ph <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.GWD, aes(y = f_height, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  annotate("text", x = 0.8, y = 130, label = "(c)", size = 5, fontface = 2) +
...
WSF.1.ph <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
 
  My_Theme +

  
    geom_jitter(data = DAT.plot.WSF, aes(y = f_height, color = var), 
            height = 0.2,
            size = 0.6, alpha = 0.3, show.legend = FALSE) +
  ...
  theme(axis.text.y = element_text(size = 12), axis.title.y = element_text(size = 14, vjust = 0), axis.title.x = element_text(size = 14)) +
  
  labs(x = NULL, y = "Height (cm)\n", title = "WSF") +
  
  scale_x_discrete(labels=xtext) +
  
  annotate("text", x = 0.8, y = 130, label = "(a)", size = 5, fontface = 2) +
...
DSF.2.ph <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
  ...
  annotate("text", x = 0.55, y = 17.5, label = "(b)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -17.5, label = "-4.18 %", size = 4, fontface = 2) +
  
  
  scale_y_continuous(breaks = c(-20, -15, -10, -5, 0, 5, 10, 15, 20), position = "left") +
  coord_cartesian(ylim=c(-20, 20))

### GWD plot 2

GWD.2.ph <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  annotate("text", x = 0.55, y = 17.5, label = "(c)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -17.5, label = "1.43 %", size = 4, fontface = 2) +
...
WSF.2.ph <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  labs(x = NULL, y = "\u0394 height (cm)\n", title = "WSF") +

  #labs(x = "", y = expression(bold(atop(NA, atop(textstyle("\u0394 biomass due to eCO"[2]*" (%)"~"\n")))))) +
  
  scale_x_discrete(labels = xtext) +
  
    theme(axis.text.y = element_text(size = 14), axis.title.y = element_text(size = 14,  vjust = 0)) +
  
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  
  annotate("text", x = 0.55, y = 17.5, label = "(a)", size = 5, fontface = 2) +
  
  annotate("text", x = 1.3, y = -17.5, label = "4.56 %", size = 4, fontface = 2) +
...
library(ggpubr)
library(egg)
fig <- egg::ggarrange(WSF.1.ph, DSF.1.ph, GWD.1.ph, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 
...
library(ggpubr)
library(egg)
fig2 <- egg::ggarrange(WSF.2.ph, DSF.2.ph, GWD.2.ph, ncol = 3, nrow = 1, widths = c(1, 1, 1)) 

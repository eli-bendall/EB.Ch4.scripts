...
DSF.1.lma <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 11.5, label = "b", size = 4, fontface = 2) +
...
GWD.1.lma <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 11.5, label = "c", size = 4, fontface = 2) +
...
WSF.1.lma <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var))+
  ...
  annotate("text", x = 0.8, y = 11.5, label = "a", size = 4, fontface = 2) +
...
DSF.2.lma <- ggplot(subset(x.stats, veg %in% c("DSF")), aes(x = var)) +
 ...
 # scale_colour_manual(values = rhg_cols2) +
  ...
  annotate("text", x = 0.6, y = 1.25, label = "b", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-9.79 %", size = 3, fontface = 2) +
...
GWD.2.lma <- ggplot(subset(x.stats, veg %in% c("GWD")), aes(x = var)) +
  ...
  annotate("text", x = 0.6, y = 1.25, label = "c", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-8.80 %", size = 3, fontface = 2) +
...
WSF.2.lma <- ggplot(subset(x.stats, veg %in% c("WSF")), aes(x = var)) +
  ...
  scale_x_discrete(labels = xtext) +
    ...
    theme(axis.text.y = element_text(size = 10), axis.title.y = element_text(size = 12, vjust = 4)) +
  
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  
  annotate("text", x = 0.6, y = 1.25, label = "a", size = 4, fontface = 2) +
  
  annotate("text", x = 1.3, y = -2.25, label = "-8.61 %", size = 3, fontface = 2) +
